...
const Modaladd = ({ isvisible, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
      <div className="bg-[#2D383C] text-white p-6 rounded-lg shadow-lg w-[500px]">
      <button className="absolute text-white text-xl" onClick={onClose}>✖</button>
      <h2 className="text-2xl font-bold text-center mb-4">Tambah Pengguna</h2>
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="w-full p-3 bg-[#39ACE7] rounded text-white">Simpan</button>
        </form>
      </div>
    </div>
  );
